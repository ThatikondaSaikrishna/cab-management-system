...
class RegistrationForm(FlaskForm):
    ...
    gender = StringField('Gender', validators=[DataRequired(), Length(4, 6, message='Between 4 to 6 characters')])
    login = TimeField('LogIn', validators=[DataRequired()])
    logout = TimeField('LogOut', validators=[DataRequired()])
    password=PasswordField("Password",validators=[DataRequired(),Length(4),EqualTo('confirm',message='password must match')])
    confirm=PasswordField('Confirm Password',validators=[DataRequired(),Length(4)])
    hno = StringField("H.No/Street", validators=[DataRequired(), Length(10, 200)])
    address = StringField("Address", validators=[DataRequired(),Length(10,200)])
